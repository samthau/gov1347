...
chl_log_base <- glm(cbind(chl_votes, floor(VEP)-chl_votes) ~ rdi_q2 + gdp + state + avg_poll, 
               data = reg_df %>% filter(year < 2020, incumbent_party == FALSE), family=binomial)

inc_log_cont <- glm(cbind(inc_votes, floor(VEP)-inc_votes) ~ rdi_q2 + gdp + state + avg_poll+ inc_party, 
                    data = reg_df %>% filter(year < 2020, incumbent_party == TRUE), family=binomial)

chl_log_cont <- glm(cbind(chl_votes, floor(VEP)-chl_votes) ~ rdi_q2 + gdp + state + avg_poll + inc_party, 
                    data = reg_df %>% filter(year < 2020, incumbent_party == FALSE), family=binomial)
...
##
## do regressions for in sample fit
dem_log_base <- glm(cbind(D, floor(VEP)-D) ~ rdi_q2 + gdp + state + avg_poll, 
                    data = reg_df %>% filter(year < 2020, party == "democrat"), family=binomial)

rep_log_base <- glm(cbind(R, floor(VEP)-R) ~ rdi_q2 + gdp + state + avg_poll, 
                    data = reg_df %>% filter(year < 2020, party == "republican"), family=binomial)

dem_log_cont <- glm(cbind(D, floor(VEP)-D) ~ rdi_q2 + gdp + state + avg_poll+ incumbent_party, 
                    data = reg_df %>% filter(year < 2020, party == "democrat"), family=binomial)

rep_log_cont <- glm(cbind(R, floor(VEP)-R) ~ rdi_q2 + gdp + state + avg_poll + incumbent_party, 
                    data = reg_df %>% filter(year < 2020, party == "republican"), family=binomial)
...
## load and append demographic data
...
summary(inc_log_demo)
summary(chl_log_demo)
