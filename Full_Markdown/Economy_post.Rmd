...
This election map is somewhat surprsing: again, it would appear that a massive GDP drop off in the second quarter dooms President Trump. This would be one of the worst results in history of the United States. We can look at the vote margins for Trump in a few states:

```{r table, echo=FALSE,  warning = FALSE, results = 'asis', message = FALSE}
out <- output[1:8,]
out <- out[,c(2,1,3,4)]
...
Based on this model, Trump is not only consistently losing, but consistently getting routed. One potential problem with this model is that each state is independent: it could be that in years where the economy is in bad shape, certain states respond in similar ways across time. 
