...
A number of states are quite close, in ways that match our general intutitions about the race. Florida, Georgia, and Ohio are all close, which matches the general consensus about the race. However, it also shows Biden winning by 10 point in Pennsylvannia, which seems unlikley given how close a race there is in Ohio in this model. We also should look at the distribution of the electoral college and the popular vote. 
...
[^3]: A model that is just the mean predictor. 
...
[^5]: One thing to note about this is that another way to measure uncertainty would have been to use the standard deviations from each state in individual polls, rather than in the average. I decided against this because individual polls can be quite noisy, and poll aggregators like 538 do a good job of putting together a trend line that removes a lot of the noise. 

[^6]: See G. Elliot Morris, Nate Cohn, and Nate Silver's twitter feeds for examples, along with thisi [blog post](https://statmodeling.stat.columbia.edu/2020/08/31/problem-of-the-between-state-correlations-in-the-fivethirtyeight-election-forecast/) from Andrew Gelman.