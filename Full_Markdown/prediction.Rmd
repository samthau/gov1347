...
## Introduction
At the time of writing, we are two days away from the 2020 presidential election between Donald Trump and Joe Biden. Many believe that this may be the most important election in the history of the United States. The question on everyone's mind is the same: who is going to win? I'll first show the results of the model, and then explain the mechcanics going on underneath the hood. There are three parts to the model:

1. Estimating turnout in each state. 

2. Estimating vote share for each candidate in each state. 

3. Simulating the election based on the estimated parameters. 

I'll go through each section in turn, and then return to the results at the end of the post to decide what I think of this model. I believe that this model gives reasonable predictions, but there are oddities that point to posisble problems that should be discussed. 
...
While these numbers do seem quite high, it is important to remember that these values are squared. Numbers that are on the order of ten to the twelfth represent being off by millions of votes, which in an election of roughly 120 million votes is quite accurate. The final thing to do with this model is predict the turnout for each state in 2020, which we will use later when we simulate the election.
...
Estimating vote share happens roughly the same way estimating turnout, just with a different generalized linear model. Similar to many of the models I have used throughout this blog, I am using a two sided model based on party incumbency status[^4].  I still used a pooled model across all states and years that I have complete data for. 

For both the incumbent and the challenger models, I use a binomial regression, estimating the fraction of the total votes that each candidate will win. The regression uses demographic data, polling data, previous eleciton results, economic data, along with party and state fixed effects. We can take a look at both the incumbent and challenger models. First, the incumbent model:

```{r, echo=FALSE,  warning = FALSE, message = FALSE, class.output="scroll-100" }
 summary(inc_vote_model)
```

We can also look the challenger model:

```{r, echo=FALSE,  warning = FALSE, message = FALSE, class.output="scroll-100" }
 summary(chl_vote_model)
```

Again, for both models, the residual deviances are orders of magnitude greater than the null deviances. One thing to note is that the incumbent model seems to have a better fit in sample than the challenger model, with a null deviance of `r format(inc_vote_model$deviance, scientific = FALSE)` against `r format(chl_vote_model$deviance, scientific = FALSE)`. Unsurprinsingly, the coefficient on the polling average is large and positive, relative to other coefficients. Interestingly, the economic data coefficients are positive in the in incumbent model, but negative in the challenger model, which makes some intuitive sense. An incumbent party with a good economy is likely to have better success with voters[^5].
...
A number of states are quite close, in ways that match our general intutitions about the race. South Carolina and Texas are both on razor thin margins. This model shows a blowout for Biden in a number of key states: he is up by more than ten points in Michigan, Florida, and Pennsylvannia. He also narrowly wins South Carolina, and nearly pulls out a victory in Texas. We also should look at the distribution of the electoral college and the popular vote. 
...
This plot shows Biden with a commanding lead in the national popular vote, winning above 50% of the vote in essentially every contest. There is a electoral college and popular vote split in `r pop_vote_split` of the 10,000 contests, meaning that Trump only wins when he happens to win the popular vote. 

## Initial Model Assessment
While this model is much more sophisticated than anything else that I have built this past semester, I think it is still flawed. The main flaw that I see is the state to state correlation: a blowout in Pennsylvannia should show that Biden is winning handily in the Midwest, and yet Ohio is still quite close.  

At the same time, this model has some pretty interesting features, namely the state to state correlation (even if it's not quite right). This is a feature that is notoriously hard to get right[^6], so I'm not particularly concerned that if the correlations are off. I wish I had more time to tune this, because I feel like this model overall understates the value of the electoral college (which in part comes from states voting similarly). 

All in all, this model gives Biden an extremely high chance of victory, which does make sense given the national enviornment. This model is quite certain of the victory, but everyone should remember that events with small probabilities do happen occasionally, no matter how small the probability is. However, what this suggests is that for Donald Trump to win re-electoin, he needs a massive polling error in his favor, or he needs courts to allow him to conduct extra-legal shenanigans that throw out votes. 

One thing to remember is this: I once heard that all models are wrong, but some models are useful. Undoubtedly, this model will be wrong, but it can still tell us a fair amount about what we expect to happen in the election. 

## Appendix: Data
Data for this model came from a number of places. In some situations, I also scaled some of the data so that different measures had approximately the same magnitude. There are a number of data sources used throughout the model.

1. Polling data. For historical data, this was an average of state level polls over the final three weeks of the election. This is a very crude poll aggregation, in averaging over time. The choice of three weeks comes from striking a balance: using polls that are recent enough, but also making sure not to limit the amount of data I have. Averaging over time is a strategy used by both [FiveThirtyEight](https://fivethirtyeight.com/features/how-fivethirtyeights-2020-presidential-forecast-works-and-whats-different-because-of-covid-19/) and [The Economist](https://projects.economist.com/us-2020-forecast/president/how-this-works). For 2020 data, I used the smoothed polling average from [FiveThirtyEight](https://projects.fivethirtyeight.com/2020-election-forecast/?cid=rrpromo), again taking the average over the final three weeks. 

2. Economic data. This comes both from sources provided in class, and from the Bureau of Economic Analysis. I used two pieces of data: second quarter GDP at the national level, and real disposible income at the state level. Real disposible income data comes from [“Quarterly Personal Income By State.”](https://apps.bea.gov/itable/iTable.cfm?ReqID=70&step=1). GDP data comes from class. The reasoning for including a mix of both economic indicators and levels is to cover the widest range of possible impact on voting behavior, and to make sure that a single indicator from 2020 does not completely skew the results. Given that we are in a recession, something like GDP will show a massive loss in the second quarter, while real disposible income will show a huge gain (due to the CARES Act). 

3. Demographic data. I use a range of demographic data at the state level, as provided in class. In both the turnout and vote share esimation regressions, I used the same set of data: fraction of the population that is white, black, hispanic, asian, age 20 to 30, age 30 to 45, and age 45 to 56. There are other race or age categories that I could include, but I wanted to avoid collinearity - you can figure out roughy how many people there are over the age of 65 by adduing up all the other categories. This data came from the dataset provided in class. One problem in this case is that the dataset I was using only had demographic data up to 2018, which is what I substituted for 2020. 

4. Indicators. These are dummy variables, primarily for party and state. Given the use in a pooled model, they are meant to capture time invariant fixed effects. 

5. Lagged data. This includes lagged vote share data, along with lagged turnout data when estimating turnout. 

One important thing I did early on in the process was scale a large amount of my data to have a mean of zero and standard deviation of one, within each year. I scaled previous turnout, the demographic and real disposible income data based on each year[^7]. This was primarily to ensure that past turnout data did not completely overwhelm everything else, as it was on a completely different order of magnitude. I intentionally did not scale the polling data, because I needed the variances in raw form to incorporate into the model later. 


[^1]: There are some assumptions that should hold for a variable to be distributed poisson, and I believe that most of them should hold. The first is that individual votes being cast happens independently from one another (for example, me casitng a vote does not make you in particular any more or less likely to cast a vote). Second, the mean should be roughly equal to the variance, which in this case it is not. The mean of total votes cast is roughly $2*10^7$, while the variance is roughly $5.6*10^12$, meaning that we have overdispersion. For simplicity in the simulation process, I decided to stick with using a poisson regression, even if it does mean that the variance in turnout will be lower. 
...
[^7]: See G. Elliot Morris, Nate Cohn, and Nate Silver's twitter feeds for examples, along with thisi [blog post](https://statmodeling.stat.columbia.edu/2020/08/31/problem-of-the-between-state-correlations-in-the-fivethirtyeight-election-forecast/) from Andrew Gelman.

[^8]: So that within each year, the mean value is 0 and the standard deviation is 1. I did not scale the GDP as it would all be set to mean 0, as it was the same for all states. 