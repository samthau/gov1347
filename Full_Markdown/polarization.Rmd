...
We can first look at the regression results to think about in sample fit. We can first look at the two sided model based on incumbency, starting with the incumbenct party side:
...
Then, compare to the challenger party side:

```{r, echo=FALSE,  warning = FALSE, message = FALSE, class.output="scroll-100" }
summary(chl_log_cont)
```

There are a few things to note when comparing these results to previous iterations. First, the coeffiicent on the party is statistically significant, suggesting that this was a smart addition to the model. Second, the deviances are still quite high. For both the incumbent and challenger, the null deviance is an order of magnitude larger than the residual deviance, which is a good sign, but the residual deviance is still extremely large. This suggests an all together mediocre fit, meaning that improvements should be made for next week's final model. We can also look at the two sided model based on a Republican and Democrat split. First, look at the Republican side:

```{r, echo=FALSE,  warning = FALSE, message = FALSE, class.output="scroll-100" }
summary(rep_log_cont)
```

And then also the Democrat side. 

```{r, echo=FALSE,  warning = FALSE, message = FALSE, class.output="scroll-100" }
summary(dem_log_cont)
```

We have the same results with the null deviances being an order of magnitude larger than the residual deviances, but the residual deviances still being on the order of ten to the sixth. One thing to note is that both sides of the party based model have higher residual and null deviances than the incumbency based model. While all of this gives us a general idea of in sample fit, the more interesting question is that of out of sample prediction. 
...
The models tend to miss similar states. In recent years, they seem to have had trouble with what we would usually call swing states, like Florida, Pennsylvannia and Wisconsin. There is no clear patter here, which suggests we are missing a host of variables. These could be demographics, or some sort of uniform swing[^4].
...
There's just one problem with this approach, that's specific to modeling the 2020 election. The variance on the predicted voting probabilites for 2020 are *tiny.* While most of the predictions on are on the order of 10 to the -1 or -2, many of the variances are on the order of 10 to the -6 or smaller. This means that if the voting probability is the only thing that has a variance, we still have an essentially deterministic model. 
...
For the current election, Biden clearly has a commanding lead. There is very, very little overlap. In all, Joe Biden wins `r victors[1,2]` out the 10,000 simulations, while Trump wins in none of them. The electoral college results in a tie the remaining `r victors[2,2]` times[^6]. Biden wins an average of `r round(mean(ec_wins$Biden), digits = 2)` electoral votes, while Trump wins an average of `r round(mean(ec_wins$Trump), digits = 2)`. 
...
[^6]: This is when absolute chaos occurs. 

[^7]: States that are similar in terms of COVID outcomes, or in terms of demographics will have similar turnout. 
